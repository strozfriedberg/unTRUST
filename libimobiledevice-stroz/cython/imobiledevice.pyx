...
# Added by Stroz Friedberg for unTRUST application on 8/8/14 - allows for receiving data through file_relay in Cython.    
    cpdef bytes receive(self, uint32_t size):
        cdef:
            ...
            bytes result
        try:
            self.handle_error(idevice_connection_receive(self._c_connection, c_data, size, &bytes_read))
